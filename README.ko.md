...
AI 에이전트가 주어진 예산(시간 또는 루프 횟수) 동안 절대 멈추지 않고, 스스로 피드백 루프를 돌며 작업 품질을 끌어올리는 스킬.
...
1. **예산이 소진될 때까지 절대 멈추지 않는다** — `min_required_minutes` 또는 `min_required_loops` 동안 능동적 작업만 수행
...
기존의 3개 모놀리식 파일은 아이디어가 쌓일수록 거대해져서:

- 전체를 컨텍스트에 로드하면 토큰 낭비
- 관련 항목만 찾아 읽기 불가능
- 세션이 길어질수록 검색 효율 급감

KB 시스템은 각 아이디어/결정/증거를 개별 Markdown 파일로 저장하고, YAML frontmatter (태그, 상태, 신뢰도 등)로 메타데이터를 붙여서 검색 서브에이전트가 필요한 것만 정확히 꺼내 쓸 수 있게 합니다.
...
도메인에 따라 증거 유형, 소스 계층, 태그 체계, 테스트 방법이 자동으로 적응합니다. 상세 매핑은 `references/DOMAIN-TABLES.md`에 정의되어 있습니다.
...
**횟수 기반 (시간 제한 없음):**
```
피드백 루프 10회 돌려서 이 함수의 에러 처리를 개선해.
```

**복합 (시간 + 횟수):**
```
최소 20분 이상, 루프 5회 이상 돌려서 이 쿼리를 최적화해.
```

시간, 루프 횟수, 또는 둘 다 지정 가능합니다. 종료 조건이 충족될 때까지 피드백 루프를 돌립니다.

### 종료 모드

| `min_required_minutes` | `min_required_loops` | 동작 |
|---|---|---|
| 설정 | — | **시간 기반**: 시간이 다 될 때까지 실행 |
| — | 설정 | **횟수 기반**: 정확히 N회 루프 실행 |
| 설정 | 설정 | **복합**: 두 조건 모두 충족될 때까지 실행 |

둘 다 미설정 시 기본값 5분이 적용됩니다.
...
리포트가 위에서부터 최신순으로 쌓입니다. 각 리포트의 첫 줄에 해당 리포트의 줄 수가 적혀 있어서, 필요한 만큼만 단계적으로 읽을 수 있습니다.
...
과거 리포트를 참조할 때는 상대좌표(`K-reports-below`, `line N below`)를 사용하므로, 로그가 정리되거나 압축되어도 참조가 깨지지 않습니다.

## 멀티 세션 사용

이전 세션의 `work-log.md`와 `kb/` 디렉터리가 남아 있으면, 다음 세션에서 이어서 작업할 수 있습니다. 에이전트가 `kb/_index.md`를 읽고 검색 서브에이전트로 최근 노트를 확인한 뒤, 이전 기록을 복원하고 이어서 피드백 루프를 돌립니다.
...
[Agent Skills 오픈 스펙](https://agentskills.io/specification)을 따르며, 이 스펙을 지원하는 모든 에이전트에서 동작합니다.
